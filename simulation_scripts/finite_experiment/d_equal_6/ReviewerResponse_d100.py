...
Nseq = [1000, 10000] # Sequence of sample sizes to try
...
def CheckRes(x, i):
    ...
    J_AS = [
        np.mean([jaccard(a.difference({'E'}), AN_Y) for a in x2[j]]) for j in range(len(Nseq))
    ]
    J_UT_IGSP = [
        np.mean([jaccard(a.difference({'E'}), AN_Y) for a in x4[j]]) for j in range(len(Nseq))
    ]
    J_AS_IAS = [
        np.mean([jaccard(a.difference({'E'}), S_AS) for a in x2[j]]) for j in range(len(Nseq))
    ]
    J_UT_IGSP_IAS = [
        np.mean([jaccard(a.difference({'E'}), S_AS) for a in x4[j]]) for j in range(len(Nseq))
    ]
    IsInvariant_AS = [
...
# database = shelve.open('output/finite_experiment/d_equal_100/ReviewerResponse')  # Change file name according to value of alpha_0
# database['data'] = data
