...
def CheckRes(x, i):
    ...
    data_out = pd.DataFrame([Nseq, IsInvariant_AS, IsInvariant_UT_IGSP, Plevel, Plevel_UT_IGSP, Pempty, Pempty_UT_IGSP, J_AS, J_UT_IGSP, [len(AN_Y)] * len(Nseq), [len(S_AS)] * len(Nseq)]).transpose()
    data_out.set_axis(['N', 'IsInvariant_AS', 'IsInvariant_UT_IGSP', 'Plevel', 'Plevel_UT_IGSP', 'Pempty', 'Pempty_UT_IGSP', 'J_AS', 'J_UT_IGSP', 'lenAN', 'lenTrueS'], axis = 1, inplace = True)
...
#### Save results ####
# database = shelve.open('output/finite_experiment/d_equal_6/ReviewerResponse')  # Change file name according to value of alpha_0
# database['data'] = data
...
# d100     = shelve.open('output/finite_experiment/d_equal_100/ReviewerResponse')
# data_100 = d100['data']
# obj_100  = d100['objlist']
# d100.close()
d6       = shelve.open('output/finite_experiment/d_equal_6/ReviewerResponse')
data   = d6['data']
objlist    = d6['objlist']
d6.close()
...
plotdata = pd.DataFrame(
    {"Method": np.repeat(['IAS', r'IAS$_\text{est.\ graph}$'], [data.shape[0]] * 2),
    "N": data['N'].tolist() * 2,
...
import matplotlib.pyplot as plt
palette = {"IAS": "#F8766D",
           r'IAS$_\text{est.\ graph}$': "#00BFC4"}
plt.clf() ; plt.cla()
sns.boxplot(x = 'N', y = 'Jaccard similarity to AN_Y', hue = 'Method', data = plotdata, showmeans = False, palette = palette)
plt.ylabel(r'Jaccard similarity to $AN_Y$')
plt.xticks(ticks = [0, 1, 2, 3], labels = [r'$10^2$', r'$10^3$', r'$10^4$', r'$10^5$'])
# plt.show()
# plt.savefig('output/finite_experiment/figures/ReviewerResponse.svg')
plt.savefig('output/finite_experiment/figures/ReviewerResponse.pdf')